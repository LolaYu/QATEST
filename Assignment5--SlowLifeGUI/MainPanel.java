 ...
 */
	...
	int size = _size;
	int leftX = x - 1; // int leftX = (x - 1) % size;
	int rightX = (x + 1) % size;
	int upY = y - 1; // int upY = (y - 1) % size ;
	int downY = (y + 1) % size;
...
// Modify getNumNeighbors() method for junit test.
// Add "Cell[][] _cells" in arguments.
    public int getNumNeighborsForTest(int x, int y, Cell[][] _cells) {
    	int size = _size;
    	int leftX = x - 1; // int leftX = (x - 1) % size;
    	int rightX = (x + 1) % size;
    	int upY = y - 1; // int upY = (y - 1) % size ;
